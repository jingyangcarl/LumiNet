...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        ...
        for module in self.input_blocks:
            ...
            # exit(0)
            hs.append(h)
        # print("fool! that is the shape of the context! at input",context[:,:77,:].shape)
        h = self.middle_block(h, emb, context)

        if control is not None:
            h += control.pop()
        
        # condtext = self.control_model.new_context
...
class ControlLDM(LatentDiffusion):
    ...
    def add_new_layers(self):
        self.control_model.add_latent_prior()
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_concat'] is None:
            ...
            cond_txt = self.control_model.new_context #for attn in the UNet
            #only for the test
        ...
        if self.crossattn_start: #here we also train the cross-attan in the input layer if has any
            for block in self.model.diffusion_model.input_blocks:
                for layer in block:
                    # print(layer)
                    # for layer in block:
                    if isinstance(layer, SpatialTransformer):  # Check if the layer is a cross-attention layer
                        params += list(layer.parameters())  # Collect parameters
                        print("!!! The Input-CRA layer loaded sucessfully")

        if self.crossattn_mid: #here we also train the cross-attan in the middle layer if has any
            for layer in self.model.diffusion_model.middle_block:
                # print(layer)
                # for layer in block:
                if isinstance(layer, SpatialTransformer):  # Check if the layer is a cross-attention layer
                    params += list(layer.parameters())  # Collect parameters
                    print("!!! The Middle-CRA layer loaded sucessfully")

        if self.crossattn: #here we only test the output layer
            for block in self.model.diffusion_model.output_blocks:
                for layer in block:
                    if isinstance(layer, SpatialTransformer): #here we train the cross-attan in the output layer
                        params += list(layer.parameters())  # Collect parameters
                        print("!!! The CRA layer loaded sucessfully")
