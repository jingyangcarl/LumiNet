...
class LatentIntrinsc(torch.nn.Module):
    def __init__(self, device='cuda'):
        ...
        self.light_decoder = nn.Sequential(nn.Linear(16, 128), nn.SiLU(), nn.Linear(128, 128))
    ...
    def forward(self,x):
        ...
        intrinsic_input_o = intrinsic_input[1]
        vectors_normed = F.normalize(extrinsic_input, p=2, dim=1)             # (100, 16)
        ground_truth_normed = F.normalize(extrinsic_ref, p=2, dim=1)   # (16,)
       
        similarities = torch.matmul(vectors_normed, ground_truth_normed.T)  # (100,)
