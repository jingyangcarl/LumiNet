    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # print((v_hat_t * (sigmas[i] -  sigmas[i + 1])).abs().mean())
...
device = "cuda"
pipe = FluxRFInversionPipeline.from_pretrained("black-forest-labs/FLUX.1-schnell", torch_dtype=torch.bfloat16,device_map="balanced")
# pipe.reset_device_map()
# pipe = pipe.to(device)
img_path = IMAGE_PATH #Path to the 
init_image = load_image(img_path).resize((1024,1024))
prompt = "photorealistic image"
## You may need to adjust the hyperparameters to achieve better visual results, as the optimal values often vary depending on the input images.
images = pipe(prompt=prompt, image=init_image, num_inference_steps=28, strength=1, guidance_scale=0.0, eta= 0.95,start_timestep = 0,stop_timestep = 19).images[0]
images.save(img_path+'_RF.png')
